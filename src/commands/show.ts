...
async function showProblemInternal(node: IProblem): Promise<void> {
    try {
        ...
        if (leetCodeManager.getStatus() === UserStatus.Guest) {
            // If fetching failed in Guest mode, try to open in browser
            const choice: vscode.MessageItem | undefined = await vscode.window.showErrorMessage(
                "Failed to fetch problem description. Open in browser instead?",
                DialogOptions.open,
                DialogOptions.no,
            );
            if (choice === DialogOptions.open) {
                // Try to construct URL from node
                // Assuming node.id is slug for custom problems
                // Standard URL: https://leetcode.com/problems/<slug>/
                // But wait, standard nodes have numeric IDs.
                // Custom nodes have slug as ID.
                // If it is a number, we need to find the slug. But we don't have it easily for standard nodes without fetching.
                // But for CUSTOM nodes, we used slug as ID.
                const isCustom = isNaN(Number(node.id));
                const slug = isCustom ? node.id : node.name.toLowerCase().replace(/\s+/g, "-"); // Fallback for standard nodes
                await vscode.commands.executeCommand("vscode.open", vscode.Uri.parse(`https://leetcode.com/problems/${slug}/`));
                return;
            }
        }
